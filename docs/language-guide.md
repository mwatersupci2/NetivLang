...
<edges>{
  ...
  %%   CompileOperation --> Notes["Public documentation note explaining why we lower to assembly here."]
};
```

The `<edges>` section records graph relationships. 

### The Dual-Note Architecture (Public vs. Private Notes)
NetivLang supports two distinct types of documentation notes:

1. **Private Notes**: Declared as a string in the `<meta>` section (e.g., `notes: "A standard entry point."`). These are private to compiler queries and AI parsers, and do not appear in visual dependency diagrams.
2. **Public Notes**: Declared inside the `<edges>` section as a `Notes` or `Notes["..."]` graph node (e.g., `CompileOperation --> Notes["Public note"]`). These notes are fully graph-visible and exported as active nodes in dependency diagrams generated by `adj.mermaid`.
...
- public documentation notes (`Notes` nodes)
